...
include 'php/obtenerArticulo.php';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    <div class="container mb-3 mt-3">
        <h1 class="titulo">¡Conoce un poco más sobre nuestra empresa!</h1>
        <p class="parrafo">COINTESA es una empresa dedicada a la venta de productos de limpieza y desinfección. Nuestros productos son de la más alta calidad y están diseñados para satisfacer las necesidades de nuestros clientes. Contamos con un equipo de profesionales altamente capacitados que están comprometidos con la excelencia y la satisfacción del cliente. Nuestro objetivo es brindar un servicio de calidad y superar las expectativas de nuestros clientes.</p>
        <h2 class="titulo mb-3 mt-4">Articulos:</h2>

        <div class="row">
            <?php
            $obtener_palabras = function($texto) {$array_palabras = explode(' ', $texto);return implode(' ', array_slice($array_palabras, 0, 10));};
            foreach($arrayDatos as $articulo){
                echo '<div class="col-md-3 mb-3">
                    <div class="card">
                        <img src="data:image/jpeg;base64,' . base64_encode($articulo[4]) . '" class="card-img-top" alt="articuloimage" height="300">
                        <div class="card-body">
                            <h5 class="card-title">'.$articulo[1].'</h5>
                            <p class="card-text">'.$resultado = $obtener_palabras($articulo[3]).'...</p>
                            <a href="articulo.php?id=' . $articulo[0] . '&titulo=' . urlencode($articulo[1]) . '" class="btn btn-primary">Ver más</a>
                        </div>
                        <div class="card-footer">
                            <small class="text-muted">Publicado el: '.$fecha_formateada = date('d/m/Y h:i A', strtotime($articulo[2])).'</small>
                        </div>
                    </div>
                    </div>
                ';
